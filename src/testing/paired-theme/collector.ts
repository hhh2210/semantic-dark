...
/** Collect only Chrome computed styles; token strings and mapper output are not scoring inputs. */
export async function collectPaintObservations(
  ...
  try {
    ...
    return observationsFrom(raw, descriptors, input);
  } finally {
    await context.close();
  }
}
...
function observationsFrom(
  ...
  const definitions = new Map(input.scenes.flatMap((scene) =>
